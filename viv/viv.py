...
class VIV(object):
    ...
    def solve_omega(self, info=False, print_='else'):
        ...
        for k in self.k_table:
            ...
            if info and cond:
                print(
                    f'k = {k:.2f}\tdiscriminant = {self.discriminant(k):.2f}\t\tomega = {sorted(np.roots(self.set_polynomial_coefs(k, self.u)), reverse=True)}')
            i += 1
    ...
    def solve_omega_for_u_range(self, info=False, print_='else'):
        ...
        for u in self.u_table:
            ...
            if info and cond:
                print(
                    f'u = {u:.2f}\tdiscriminant = {self.discriminant(u):.2f}\t\tomega = {sorted(np.roots(self.set_polynomial_coefs(self.k, u)), reverse=True)}')
            i += 1
    ...
    def solve_gain(self):
        ...
        for omg, k in zip(np.array(self.omega), self.k_table):
            gain = self.A * omg ** 2 / (omg ** 2 - self.u ** 2 + 1j * self.nu * self.epsilon * omg * self.u)
    ...
    def solve_gain_u(self):
        ...
        for omg, u in zip(np.array(self.omega), self.u_table):
            gain = self.A * omg ** 2 / (omg ** 2 - u ** 2 + 1j * self.nu * self.epsilon * omg * u)
            self.gain.append(gain)
    ...
    def solve_gain_u_k(self):
        ...
        for omg in (np.array(self.u_k_table)):
            gain = self.A * omg ** 2 / (
                        omg ** 2 - self.u_lim[0] ** 2 + 1j * self.nu * self.epsilon * omg * self.u_lim[0])
            self.gain.append(gain)
            self.phase.append(np.angle(-gain, deg=False) / np.pi)
